...
export class MainScene extends Scene {
    ...
    checkHighlight(): void {
        ...
        let hoverResult = this.validHover(new Phaser.Math.Vector2(pointer.x, pointer.y));
    ...
    validHover(position: Phaser.Math.Vector2): [IDItem, number] | false {
        for (const node of this.nodes) {
            if (node.pos.distance(position) < 10) { // Assuming a proximity check
        ...
        for (const edge of this.edges) {
            if (edge.isNear(position)) { // You need to define how to check proximity to an edge
                if (this.ps === PSE.PLAY) {
