...
export class Preloader extends Scene
{
    ...
    createShapeTextures() {
        ...
        let filledTriangle = this.add.graphics({ fillStyle: { color: 0xffffff }, lineStyle: { width: 0.5, color: 0x000000 }  });
        filledTriangle.fillTriangle(centre + 0, centre - 8 * siz, centre - 7 * siz, centre + 4 * siz, centre + 7 * siz, centre + 4 * siz);
        filledTriangle.strokePath();
        ...
        this.drawCurvedPlus(plus, centre, centre, siz * 24); // Use siz * 12 to make the plus large, matching the star
