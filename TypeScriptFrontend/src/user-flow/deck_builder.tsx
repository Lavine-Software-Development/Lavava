...
const DeckBuilder: React.FC = () => {
        ...
        const fetchAbilities = async () => {
            try {
                const response = await fetch(
                    "https://172.31.8.69:5001/abilities"
                );
    ...
    const handleStartFresh = () => {
        setSelectedCounts(
            abilities.reduce(
                (counts: { [key: string]: number }, ability: Ability) => {
        ...
        setSalary(
            abilities.reduce(
                (total, ability) => total + ability.cost * 0,
                salary
            )
        ); // Reset salary to full amount
    };
    ...
    const handleSaveDeck = async () => {
        ...
        if (token) {
            ...
            try {
                ...
                const response = await fetch(
                    "https://172.31.8.69:5001/save_deck",
                    {
                        method: "POST",
                        headers: {
    ...
    const handleResetDeck = async () => {
        ...
        if (token) {
            try {
                const response = await fetch(
                    "https://172.31.8.69:5001/user_abilities",
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
                ...
                if (response.ok) {
                    ...
                    const initialCounts = userAbilities.reduce(
                        (
                            counts: { [key: string]: number },
                            ability: { name: string; count: number }
                        ) => {
    ...
    const handleButtonClick = (abilityName: string, increment: boolean) => {
        ...
        setSelectedCounts((prevCounts) => {
            const abilityCost =
                abilities.find((a) => a.name === abilityName)?.cost || 0;
            ...
            const distinctAbilities = Object.keys(prevCounts).filter(
                (name) => prevCounts[name] > 0
            ).length;
                ...
                {abilities.map((ability, index) => (
                    <button
                        ...
                        className={`ability-button ${
                            selectedCounts[ability.name] > 0 ? "selected" : ""
                        }`}
