...
from player import CreditPlayer, BasicPlayer
...
class ServerGame(JsonableTick):
    def __init__(self, player_count, gs, settings):
        ...
        if settings["ability_type"] == "credits":
            self.player_dict = {i: CreditPlayer(i) for i in range(player_count)}
        else:
            self.player_dict = {
                i: BasicPlayer(i, settings) for i in range(player_count)
            }
    ...
    def create_bot(self, player_id, ability_process):
        self.player_dict[player_id] = create_ai(
            player_id,
            self.settings,
            self.board,
            self.effect,
            self.event,
            self.get_ordered_counts,
            self.eliminate,
            ability_process,
        )
    ...
    def bots(self):
        return [
            player for player in self.player_dict.values() if isinstance(player, AI)
        ]
    ...
    def event(self, key, player_id, data):
        ...
        if event.can_use(player, data):
            ...
            if (
                len(data) == 3
                and data[2] in player.abilities
                and player.abilities[data[2]].can_afford
            ):
    ...
    def restart(self):
        self.accessibility_times = (
            self.settings["accessibility_times"]
            if self.settings["iterative_make_accessible"]
            else []
        )
    ...
    def make_events_dict(self):
        ...
        effects = make_event_effects(
            self.board, self.update_extra_info, self.settings["cannon_shot_decrease"]
        )
    ...
    def set_abilities(self, player, abilities, settings):
        ...
        self.player_dict[player].set_abilities(
            abilities, self.ability_effects, self.board, settings
        )
    ...
    def all_player_starts_selected(self):
        return all(
            [
                p.ps.state in (PSE.START_WAITING, PSE.ELIMINATED)
    ...
    def no_player_starts_selected(self):
        return all(
            [p.ps.state == PSE.START_SELECTION for p in self.player_dict.values()]
        )
    ...
    def update_timer(self):
        if self.countdown_timer > 0:
            ...
            if self.accessibility_times and self.gs.value == GSE.PLAY.value:
                if (
                    self.times[self.current_section]
                    <= self.settings["main_time"] - self.accessibility_times[0]
                ):
            ...
            if (
                self.gs.value == GSE.START_SELECTION.value
                and self.countdown_timer > 3
                and self.all_player_starts_selected
            ):
    ...
    def only_bots_remain(self):
        return all(
            [isinstance(self.player_dict[player], AI) for player in self.remaining]
        )
                ...
                if player.count == 0:
                    ...
                    if player.killer:
                        self.update_extra_info(
                            ("player_elimination", (player.id, player.killer.id))
                        )
                    else:
    ...
    def determine_ranks_from_capitalize_or_timeout(self):
        ...
        player_scores = {
            key: self.player_dict[key].rank * -1000 + val * 100 + player_nodes[key]
            for key, val in player_capitals.items()
        }
    ...
    def hundred_per_capital(self):
        ...
        # maximum of 3
        return {
            player.id: self.board.full_player_capitals[player.id]
            for player in self.player_dict.values()
        }
    ...
    def only_winner_capitals_count(self):
        ...
        return {
            player.id: int(self.board.full_player_capitals[player.id] >= 3)
            for player in self.player_dict.values()
        }
